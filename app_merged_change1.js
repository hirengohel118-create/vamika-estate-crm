...
let settings = JSON.parse(localStorage.getItem('vem_settings') || '{}');
  ...
  function renderProjects(){
    ...
    projects.forEach((p, idx) => {
      const c = document.createElement('div'); c.className='lead-card';
      c.innerHTML = `<div class="lead-row"><div class="lead-left"><div class="lead-title">${p.name}</div><div class="lead-meta">${p.config} • ${p.location || ''}</div></div><div class="card-actions">
...
function openProjectForEdit(idx){
  ...
  $('projectSize').value = p.size || '';
  $('projectPrice').value = p.price || '';
  $('projectDesc').value = p.desc || '';

  $('projectForm').style.display='flex';
  $('projectForm').setAttribute('aria-hidden','false');
}
