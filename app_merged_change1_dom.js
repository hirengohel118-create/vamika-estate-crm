...
let settings = JSON.parse(localStorage.getItem('vem_settings') || '{}');
  ...
  function renderFollowups(){
    ...
    // Inject Edit button safely (DOM-based)
    const actions = c.querySelector('.card-actions');
    if(actions && !actions.querySelector('.edit')){
      const eb = document.createElement('button');
      eb.className = 'small-btn edit';
      eb.textContent = 'Edit';
      eb.addEventListener('click', (e)=>{
        e.stopPropagation();
        openProjectForEdit(idx);
      });
  ...
  $('saveProject') && $('saveProject').addEventListener('click', ()=>{ const p={name:$('projectName').value||'Untitled', location:$('projectLocation').value, config:$('projectConfig').value, size:$('projectSize').value, price:$('projectPrice').value, desc:$('projectDesc').value}; if(editingProjectIndex!==null){
    projects[editingProjectIndex]=p;
  } else {
    projects.unshift(p);
  }
  editingProjectIndex=null; localStorage.setItem('projects',JSON.stringify(projects)); renderProjects(); $('projectForm').style.display='none'; });
...
function openProjectForEdit(idx){
  const p = projects[idx];
  editingProjectIndex = idx;
  if(document.getElementById('projectName')) document.getElementById('projectName').value = p.name || '';
  if(document.getElementById('projectLocation')) document.getElementById('projectLocation').value = p.location || '';
  if(document.getElementById('projectConfig')) document.getElementById('projectConfig').value = p.config || '';
  if(document.getElementById('projectSize')) document.getElementById('projectSize').value = p.size || '';
  if(document.getElementById('projectPrice')) document.getElementById('projectPrice').value = p.price || '';
  if(document.getElementById('projectDesc')) document.getElementById('projectDesc').value = p.desc || '';
